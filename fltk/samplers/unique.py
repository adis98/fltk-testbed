...
class UniqueSampler(DistributedSamplerWrapper):
    """
    ...
    def __init__(self, dataset, num_replicas=None, rank=None, args=(0, 10)):
        ...
        try:
            taskList = random.sample(self.ordered_indices.keys(),
                                     len(self.ordered_indices.keys()))  # sample all the tasks in a random sequence for each client
        except ValueError:
            print("More tasks to sample than the task list size!")
            exit(0)
        if rank > 0:
            self.taskList = taskList[rank - 1:len(
                taskList):self.num_replicas]  # make sure each client gets a unique partition of the tasks
            try:
                self.taskList = self.taskList[
                                :num_task]  # take only the required number of tasks from the whole set. Requires num_tasks * num_client to be <= the total number of cifar100 tasks
            except IndexError:
                print("num clients * tasks per client exceeds the total number of dataset tasks")
                exit(0)
            indices = self.ordered_indices[self.taskList[0]]
            random.shuffle(indices)
            self.indices = indices[rank - 1:len(
                indices):self.num_replicas]  # uniformly divide the indices among clients after shuffling
    ...
    def set_task(self, taskID):
        ...
        random.seed(self.rank)  # let each client shuffle its data however it wants. The task set is unique to it anyway
        random.shuffle(indices)
