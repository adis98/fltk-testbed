...
class ShuffledSampler(DistributedSamplerWrapper):
    """
    ...
    def __init__(self, dataset, num_replicas=None, rank=None, args=(0, 10)):
        ...
        try:
            ...
            random.seed(rank)
            self.taskList = random.shuffle(taskList) # The same set of tasks but different order for each client
        except ValueError:
        ...
        if rank > 0:
            ...
            self.indices = indices[rank-1:len(indices):self.num_replicas] # uniformly divide the indices among clients after shuffling
